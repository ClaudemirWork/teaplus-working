...
export default function PatternMatchCollaborativePage() {
  ...
  const renderShape = (color: string, shape: string, isActive: boolean) => {
    ...
    const colorClasses = { red: isActive ? 'bg-red-500' : 'bg-red-200', blue: isActive ? 'bg-blue-500' : 'bg-blue-200', green: isActive ? 'bg-green-500' : 'bg-green-200', yellow: isActive ? 'bg-yellow-500' : 'bg-yellow-200', purple: isActive ? 'bg-purple-500' : 'bg-purple-200', orange: isActive ? 'bg-orange-500' : 'bg-orange-200' };
    const shapeClasses = { circle: 'rounded-full', square: 'rounded-lg', triangle: 'rounded-lg transform rotate-45', diamond: 'rounded-lg transform rotate-45', star: 'rounded-full', heart: 'rounded-full' };
    return <div className={`${baseClasses} ${colorClasses[color as keyof typeof colorClasses]} ${shapeClasses[shape as keyof typeof shapeClasses]}`} />;
  };
