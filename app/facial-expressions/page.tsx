...
const confetti = (opts: any = {}) => {
  ...
  try {
    ...
    class Particle {
      ...
      velocity: { x: number; y: number }; gravity: number; friction: number;
...
export default function FacialExpressionsGame() {
  ...
  const sayLeo = useCallback(async (text: string, priority = 1) => {
    ...
    // Usa a fila e anti-spam global
    if (audio.shouldSpeak('leo-generic', 400)) {
  ...
  const handleStartIntro = useCallback(async () => {
    ...
    const audio = GameAudioManager.getInstance();
    await audio.forceInitialize().catch(() => {});
    // Introdução unificada em uma tela única
    const parts = [
    ...
    // Enfileira todas as frases com prioridade normal
    parts.forEach((p, idx) => {
  ...
  const preparePhase = useCallback((phaseIndex: number) => {
    ...
    for (let i = 0; i < phaseConfig.numRounds; i++) {
      seq.push(cardsToDisplay[Math.floor(Math.random() * cardsToDisplay.length)]);
    }
    ...
    if (isCorrect) {
      ...
      setTimeout(() => {
        ...
        if (nextIdx < targetSequence.length) {
          ...
          setSelectedCardId(null);
          setIsDisabled(false);
          sayLeo(targetSequence[nextIdx].label, 2);
        } else {
    ...
    <div className="screen-center intro-explanation">
      <div className="intro-content-wrapper">
        ...
        <div className="speech-bubble">
          ...
          <p style={{ marginBottom: 8 }}>É bem fácil! Eu vou falar uma emoção, como Feliz, ou Triste.</p>
      ...
      <button
        ...
        title={!introDone ? 'Aguarde o Leo terminar de falar' : 'Começar'}
      >
  ...
  const renderPhaseComplete = () => (
    <div className="screen-center">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.7 }} animate={{ opacity: 1, scale: 1 }} className="modal-container">
        ...
        <p>
          Pontuação: <span className="total-score-highlight">{totalScore}</span>
        </p>
  ...
  const cssStyles = `
    ...
    /* (CSS idêntico ao já enviado no arquivo original, sem mudanças visuais relevantes) */
    /* Cole aqui exatamente o mesmo cssStyles do arquivo original para manter a aparência */
  `;
  ...
  return (
    <div className={`game-container ${gameState === 'title' || gameState === 'intro' ? 'intro-mode' : ''}`}>
      ...
      <header className="game-header">
        ...
        <button
          onClick={() => {
            const enabled = GameAudioManager.getInstance().toggleAudio();
