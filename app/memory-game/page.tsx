    ...
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 mb-6">
        <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-red-400">
                <h3 className="font-semibold text-gray-800 mb-1 flex items-center"><Target size={18} className="mr-2 text-red-500"/> Objetivo:</h3>
            ...
            <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-blue-400">
                <h3 className="font-semibold text-gray-800 mb-1 flex items-center"><Gamepad2 size={18} className="mr-2 text-blue-500"/> Como Jogar:</h3>
            ...
            <div className="bg-white rounded-xl shadow-md p-4 border-l-4 border-yellow-400">
                <h3 className="font-semibold text-gray-800 mb-1 flex items-center"><Trophy size={18} className="mr-2 text-yellow-500"/> Níveis:</h3>
                <ul className="text-sm text-gray-600 list-disc list-inside">
...
const Card = ({ card, onCardClick }) => {
  ...
  return (
    <div className="perspective-1000" onClick={() => onCardClick(card.id)}>
      <div 
        className={`relative w-full h-full transform-style-3d transition-transform duration-500 rounded-lg shadow-md cursor-pointer ${isFlipped ? 'rotate-y-180' : ''}`}
        style={{ aspectRatio: '2.5 / 3.5' }}
      >
        {/* Verso da Carta */}
        <div className="absolute w-full h-full backface-hidden flex items-center justify-center bg-gradient-to-br from-teal-500 to-blue-600 rounded-lg">
          <span className="text-white text-2xl font-bold tracking-wider">TeaPlus</span>
        </div>
        {/* Frente da Carta */}
        <div className={`absolute w-full h-full backface-hidden rotate-y-180 flex items-center justify-center rounded-lg ${card.status === 'matched' ? 'bg-green-100 border-2 border-green-300' : 'bg-white'}`}>
  ...
  <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-30">
    <div className="bg-white rounded-2xl p-8 shadow-2xl text-center max-w-sm mx-4 animate-fade-in">
      ...
      <div className="space-y-3">
        {level < 4 ? (
          <button onClick={onNextLevel} className="w-full bg-teal-500 hover:bg-teal-600 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105">
            Próximo Nível
          </button>
        ) : (
          <p className="font-bold text-green-600">🎉 Você completou todos os níveis! 🎉</p>
        )}
        <button onClick={onRestart} className="w-full bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-3 px-4 rounded-lg transition-colors">
          Jogar Novamente
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [gameState, setGameState] = useState('loading'); // 'loading', 'memorize', 'playing', 'won'
  ...
  const setupGame = (currentLevel) => {
    ...
    const shuffledCards = duplicatedEmojis
      .map((emoji, index) => ({ id: index, emoji, status: 'flipped' }))
    ...
    memorizeTimeoutRef.current = setTimeout(() => {
      setCards(currentCards => currentCards.map(card => ({ ...card, status: 'hidden' })));
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-100 flex flex-col">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-4 -mt-6">
        <div className="mb-4 text-center">
          ...
          {gameState === 'memorize' && <p className="text-blue-600 font-semibold animate-pulse">Memorize as cartas!</p>}
        </div>
