...
const ToneButton = ({ tone, onClick, isActive }) => (
    <motion.button
        onClick={onClick}
        className={`tone-button ${tone.color} ${isActive ? 'active' : ''}`}
        whileTap={{ scale: 0.9 }}
    >
...
export default function AuditoryMemoryGame() {
    ...
    const tones = useMemo(() => [
        { frequency: 261.63, color: 'bg-red-500', name: 'Dó' },
        { frequency: 293.66, color: 'bg-blue-500', name: 'Ré' },
        { frequency: 329.63, color: 'bg-green-500', name: 'Mi' },
        { frequency: 349.23, color: 'bg-yellow-500', name: 'Fá' },
        { frequency: 392.00, color: 'bg-purple-500', name: 'Sol' },
        { frequency: 440.00, color: 'bg-pink-500', name: 'Lá' },
    ], []);

    const gameSpeed = useMemo(() => Math.max(200, 800 - (score / 50)), [score]);
    ...
    const nextTurn = useCallback(() => {
        ...
        if (nextIndex >= currentSequence.length) {
            const nextTone = Math.floor(Math.random() * tones.length);
            currentSequence = [...currentSequence, nextTone];
            setSequence(currentSequence);
            nextIndex = 0;
            setCurrentIndex(0);
        }

        setActiveTone(currentSequence[nextIndex]);
        playTone(tones[currentSequence[nextIndex]].frequency);

        clearTimeout(gameTimer.current);
        gameTimer.current = setTimeout(() => {
            if (gameState === 'playing') {
                 handleUserChoice(-1); // -1 indica que o tempo esgotou
            }
        }, gameSpeed + 500);

    }, [currentIndex, sequence, tones, playTone, gameSpeed, gameState, lives]);
    
    useEffect(() => {
        if (gameState === 'playing' && activeTone === null) {
           const timeout = setTimeout(nextTurn, 500);
           return () => clearTimeout(timeout);
        }
    }, [gameState, nextTurn, activeTone]);
    ...
    const handleUserChoice = (chosenIndex) => {
        ...
        clearTimeout(gameTimer.current);
        const correctIndex = sequence[currentIndex];
        ...
        if (chosenIndex === correctIndex) {
            ...
            setCombo(1);
            // Pausa breve para o jogador perceber o erro antes de reiniciar a sequência
            setTimeout(() => {
                 if (lives - 1 > 0) {
                     setCurrentIndex(0); // Reinicia a sequência atual
                 }
            }, 1000);
        }
    ...
    const cssStyles = `
        ...
        /* --- UI DO JOGO --- */
        .game-header { padding: 15px; display: flex; justify-content: space-between; align-items: center; width: 100%; max-width: 800px; margin: 0 auto; }
        ...
        .game-board { flex-grow: 1; display: flex; align-items: center; justify-content: center; }
        .tone-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; }
        @media (max-width: 600px) { .tone-grid { grid-template-columns: repeat(2, 1fr); } }
        
        .tone-button { width: 120px; height: 120px; border-radius: 20px; border: none; cursor: pointer; position: relative; overflow: hidden; transition: transform 0.1s; }
        .button-shine { position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: rgba(255,255,255,0.2); transform: rotate(45deg); pointer-events: none; }
        .button-label { font-size: 1.5rem; font-weight: 700; color: white; text-shadow: 0 2px 5px rgba(0,0,0,0.3); }
        .tone-button.active { animation: pulse 0.5s; }
    ...
    return (
        <div className="game-container">
            ...
            <AnimatePresence mode="wait">
                ...
                {gameState === 'playing' && (
                     <main className="game-board" key="playing">
                        <AnimatePresence>
                            {combo > 2 && (
                                <motion.div 
