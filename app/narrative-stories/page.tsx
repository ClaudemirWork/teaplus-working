...
// --- INTERFACES E DADOS (sem alterações) ---
interface Card {
    id: string;
    displayLabel: string;
    image: string;
...
const conjugateVerb = (infinitive: string, person: 'eu' | 'voce' | 'ele_ela'): string => {
    const irregulars: { [key: string]: { [key: string]: string } } = { 'ler':{eu:'leio',voce:'lê',ele_ela:'lê'},'ver':{eu:'vejo',voce:'vê',ele_ela:'vê'},'ir':{eu:'vou',voce:'vai',ele_ela:'vai'},'ser':{eu:'sou',voce:'é',ele_ela:'é'},'estar':{eu:'estou',voce:'está',ele_ela:'está'},'ter':{eu:'tenho',voce:'tem',ele_ela:'tem'} };
    if(irregulars[infinitive]) return irregulars[infinitive][person];
    const ending = infinitive.slice(-2); const root = infinitive.slice(0,-2);
    if(ending==='ar'){ if(person==='eu')return root+'o'; if(person==='voce'||person==='ele_ela')return root+'a';}
    if(ending==='er'){ if(person==='eu')return root+'o'; if(person==='voce'||person==='ele_ela')return root+'e';}
    if(ending==='ir'){ if(person==='eu')return root+'o'; if(person==='voce'||person==='ele_ela')return root+'e';}
    return infinitive; 
};
...
export default function HistoriasEpicasGame() {
    ...
    const buildSentence = useCallback((phrase: Card[], level: Level): string => {
    ...
    const handleCardSelection = (card: Card) => {
        ...
        setCurrentPhrase(prevPhrase => [...prevPhrase, card]);
    };
    ...
    useEffect(() => {
        ...
        if (nextStepIndex >= level.structure.length) {
            ...
            if (level.isRecognitionOnly) {
                instruction = `Nível ${level.level}: ${level.description}.`;
            } else {
                 switch(nextCategory) {
                    case 'personagens': instruction = `Nível ${level.level}: Escolha um personagem.`; break;
                    case 'acoes': instruction = subject ? `O que ${subject.displayLabel.toLowerCase()} faz?` : "Escolha uma ação."; break;
                    case 'objetos': instruction = "Com o quê?"; break;
                    case 'lugares': instruction = "Onde?"; break;
                    case 'tempo': instruction = "Quando?"; break;
                    default: instruction = "Continue a história!";
                }
            }
                    ...
                    {gameState === 'playing' ? (
                        ...
                        <div className='text-center mt-6'>
                            <button onClick={handleNext} className="px-10 py-4 bg-gradient-to-r from-green-500 to-blue-600 text-white font-bold text-xl rounded-full shadow-xl hover:scale-105 transition-transform">
                                {gameState === 'gameOver' ? 'Jogar Novamente' : 'Próxima Frase!'}
                            </button>
                        </div>
                    )}
                </div>
