...
interface TitleScreenProps {
  onStart: () => void;
  toggleAudio: () => void; // manter compat com props existentes
...
export const TitleScreen = React.memo(function TitleScreen({
  ...
  const handleStartIntroSpeech = async () => {
    ...
    // Garante que o navegador liberou o áudio
    await audioManager.forceInitialize();

    // Interrompe qualquer fala anterior
    audioManager.pararTodos();

    // Introdução em fila (o manager já organiza SSML e pausas)
  ...
  // Timeout para mostrar o botão "Começar" mesmo sem interação
  useEffect(() => {
    if (!userInteracted) {
      const timer = setTimeout(() => {
        if (!introSpeechComplete) setIntroSpeechComplete(true);
      }, 7000);
  ...
  return (
    <div className="relative w-full h-screen flex justify-center items-center p-4 bg-gradient-to-br from-cyan-300 via-blue-400 to-blue-600 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        {!userInteracted && !introSpeechComplete && (
          <button
            onClick={handleStartIntroSpeech}
            ...
            aria-label="Ouvir introdução da Mila"
          >
        ...
        {introSpeechComplete && (
          <button
            onClick={onStart}
            className="text-xl font-bold text-white bg-gradient-to-r from-green-500 to-cyan-500 rounded-full px-12 py-5 shadow-xl transition-all duration-300 hover:scale-110 hover:rotate-1 animate-pulse"
            aria-label="Começar aventura"
          >
            Começar Aventura
          </button>
        )}
