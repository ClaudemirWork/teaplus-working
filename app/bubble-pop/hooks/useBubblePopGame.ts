...
export function useBubblePopGame(gameAreaRef: React.RefObject<HTMLDivElement>) {
    ...
    // Simples: arrays vazios para features futuras.
    ...
    const levelConfigs = useMemo(() => [
        { level: 1, name: 'Superfície (0-10m)', depth: '0-10m', totalBubbles: 80, minePercentage: 0.05, spawnRate: 1200, oxygenDrain: 0.15, bgGradient: 'from-cyan-300 to-blue-400' },
        { level: 2, name: 'Águas Rasas (10-30m)', depth: '10-30m', totalBubbles: 50, minePercentage: 0.15, spawnRate: 1300, oxygenDrain: 0.25, bgGradient: 'from-blue-400 to-blue-500' },
        { level: 3, name: 'Zona Média (30-60m)', depth: '30-60m', totalBubbles: 35, minePercentage: 0.30, spawnRate: 1400, oxygenDrain: 0.35, bgGradient: 'from-blue-500 to-blue-700' },
        { level: 4, name: 'Águas Fundas (60-100m)', depth: '60-100m', totalBubbles: 23, minePercentage: 0.45, spawnRate: 1500, oxygenDrain: 0.45, bgGradient: 'from-blue-700 to-indigo-900' },
        { level: 5, name: 'Zona Abissal (100m+)', depth: '100m+', totalBubbles: 13, minePercentage: 0.60, spawnRate: 1600, oxygenDrain: 0.60, bgGradient: 'from-indigo-900 to-black' }
    ], []);

    const coloredBubbles = useMemo(() => ({
        air: { color: '#E0F2FE', points: 5, size: 40, speed: 1 },
        oxygen: { color: '#60A5FA', points: 15, size: 55, speed: 1 },
        pink: { color: '#F9A8D4', points: 20, size: 45, speed: 1 },
        purple: { color: '#C084FC', points: 25, size: 45, speed: 0.95 },
        yellow: { color: '#FDE047', points: 30, size: 45, speed: 1 },
        green: { color: '#86EFAC', points: 35, size: 45, speed: 0.85 },
        orange: { color: '#FB923C', points: 40, size: 45, speed: 1 },
        treasure: { color: '#FFD700', points: 50, size: 50, speed: 0.8 },
        pearl: { color: '#FFF0F5', points: 100, size: 40, speed: 0.8 }
    }), []);
    ...
    const playPopSound = useCallback(() => {
        ...
        try {
            GameAudioManager.getInstance().playSoundEffect("pop", 0.7);
    ...
    const createParticles = useCallback((x: number, y: number, color: string, isExplosion: boolean = false) => {
        ...
        const particleCount = isExplosion ? 15 : 7;
    ...
    // LÓGICA PRINCIPAL DAS BOLHAS: mais estável, fiel ao original
    useEffect(() => {
        if (!isPlaying) {
    ...
    useEffect(() => {
        ...
        const spawn = () => {
            ...
            const newBubble: Bubble = {
                ...
                x: Math.random() * (gameArea.width - bubbleConf.size),
                y: gameArea.height + bubbleConf.size,
        ...
        const interval = setInterval(() => {
            const config = levelConfigs[currentLevel - 1];
            if (bubblesSpawned < config.totalBubbles) spawn();
        }, config.spawnRate);
